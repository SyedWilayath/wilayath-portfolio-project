      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <p className={`text-lg leading-relaxed animate-fade-in transition-all duration-300 hover:scale-105 ${
          ...
          <div className={`p-8 rounded-2xl transition-all duration-500 hover:transform hover:scale-105 hover:shadow-2xl animate-fade-in ${
            darkMode ? 'bg-gray-900 border border-gray-700 hover:shadow-blue-500/20' : 'bg-gray-50 border border-gray-200 hover:shadow-blue-500/10'
          }`}
          style={{ animationDelay: '0.3s', animationFillMode: 'both' }}>
            ...
            <div className="space-y-4">
              {features.map((item, index) => (
                <div 
                  key={index} 
                  className="flex items-center space-x-3 group transition-all duration-300 hover:transform hover:translate-x-2 animate-fade-in"
                  style={{ 
                    animationDelay: `${0.5 + index * 0.1}s`,
                    animationFillMode: 'both'
                  }}
                >
                  <div className="w-2 h-2 bg-blue-600 rounded-full transition-all duration-300 group-hover:scale-150 group-hover:bg-blue-400"></div>
                  <span className={`transition-colors duration-300 ${darkMode ? 'text-gray-300 group-hover:text-white' : 'text-gray-700 group-hover:text-gray-900'}`}>
                    {item}
                  </span>
                </div>
              ))}
